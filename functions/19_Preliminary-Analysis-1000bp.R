...
myQCresults_allsamples_1000bp <- function(methylBase_temp1, path_temp1) { 
  meth_2two = tileMethylCounts( methylBase_temp1,   win.size=1000,   step.size=1000,   cov.bases = 7  ) 
  ...
  write.table(myMeLevelMatrix , 
              file = paste(path_temp1,   "4B_same-as-3B_mat-tiles-bySelfCompute.txt",  sep="/"), 
              append = FALSE, quote = FALSE, sep = "\t", eol = "\n", na = "NA", dec = ".", 
              row.names = FALSE,  col.names = TRUE, qmethod = c("escape", "double"),  fileEncoding = "")
  ...
  path_temp2_sub7 = paste(path_temp1, "7_HierarchicalClustering", sep="/")
  if( ! file.exists(path_temp2_sub7) ) { dir.create(path_temp2_sub7, recursive = TRUE) }
  tryCatch(
      myHierarchicalClustering_1_g(  mat_3three=mat_2two ,  path_temp1=path_temp2_sub7,    width1 = myNumberSamples/3 + 2 ,   height1= myNumberSamples/20 + 2 )  ,             
      error = function(err){"Error: myHierarchicalClustering_1_g"}
  )
  tryCatch(
    myTempFunction101(),
    error = function(err){"myTempFunction101_1bp_000111333"}
